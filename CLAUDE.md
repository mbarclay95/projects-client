...
| `grocery`       | `/app/grocery`       | Two tabs, Shopping List and Master List, behind `GroceryTabsComponent`'s `nz-tabs`; on mobile the tab shell is skipped for two footer buttons pointing straight at the child routes. `GroceryListItemsSignalStore` sits beside `GroceryItemsSignalStore`. The master list's filtering (search + tags) runs client-side through `filterGroceryItems` over the store's `GroceryUiState`; the Add Items picker that builds the shopping list reuses that same function over its own local `search`/`tags` signals instead, because filtering through the master list's UI state would silently re-filter the Master List tab behind it. An empty state for a user with no grocery group; each item carries a `GroceryItemUnit` (`none`/`weight`/`count`) and an optional `defaultQuantity`, rendered through `formatGroceryItemAmount` on the master list and `formatGroceryListItemAmount` on a shopping-list entry (which the picker also seeds a new entry's `quantity` from). Ticking an entry off calls the store's `markBought()`, an `update({ removeFromStore: true })` — the entry leaves the list the moment the request succeeds, with no optimistic update |
