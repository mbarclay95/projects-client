...
describe('DraftService', () => {
  ...
  it('polls every 1s while in_progress and stops once the status leaves in_progress', fakeAsync(() => {
    service.getDraft('12', 'tok');
    httpMock.expectOne(`${environment.publicApiUrl}/drafts/12?token=tok`).flush(inProgressDraft());
    tick();

    // timer(0, 1000)'s immediate tick, fired by the status transition to in_progress.
    httpMock.expectOne(`${environment.publicApiUrl}/drafts/12?token=tok`).flush(inProgressDraft());
    tick();
