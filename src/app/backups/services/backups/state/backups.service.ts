...
export class BackupsService {
  ...
  async createNewBackups(newBackup: Backup): Promise<void> {
    await firstValueFrom(
      this.http.post<Backup>(`${environment.apiUrl}/backups`, newBackup).pipe(
  ...
  async updateBackups(newBackup: Backup): Promise<void> {
    await firstValueFrom(
      this.http.put<Backup>(`${environment.apiUrl}/backups/${newBackup.id}`, newBackup).pipe(
        map((backup) => createBackup(backup)),
  ...
  async deleteBackups(backup: Backup): Promise<void> {
    await firstValueFrom(
      this.http.delete<Backup>(`${environment.apiUrl}/backups/${backup.id}`).pipe(tap(() => this.backupsStore.remove(backup.id))),
    );
  }
}
