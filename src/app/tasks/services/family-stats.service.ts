...
export class FamilyStatsService {
  ...
  ui$: Observable<FamilyStatsUi> = combineLatest([this.familiesQuery.selectActive(), this.uiSubject.asObservable()]).pipe(
    map(([family, ui]) => {
  ...
  yearBehindDisabled$: Observable<boolean> = combineLatest([this.familiesQuery.selectActive(), this.uiSubject.asObservable()]).pipe(
    map(([family, ui]) => !!family && family.minYear === ui.year),
  );

  yearAheadDisabled$: Observable<boolean> = this.uiSubject.asObservable().pipe(map((ui) => getYear(new Date()) === ui.year));
  ...
  loadFamilyStats(ui: FamilyStatsUi, familyId?: number): void {
    ...
    this.http
      .get<FamilyMemberStats[]>(`${environment.apiUrl}/family-stats?familyId=${familyId}&year=${ui.year}`)
      .pipe(
