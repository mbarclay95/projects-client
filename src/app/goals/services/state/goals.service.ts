...
export class GoalsService {
  ...
  async getAllGoals(): Promise<void> {
    ...
    await firstValueFrom(
      this.http.get<Goal[]>(`${environment.apiUrl}/goals?${queryString}`).pipe(
  ...
  async createNewGoal(goal: Goal): Promise<void> {
    await firstValueFrom(
      this.http.post<Goal>(`${environment.apiUrl}/goals`, goal).pipe(
  ...
  async updateGoal(goal: Goal): Promise<void> {
    ...
    await firstValueFrom(
      this.http.patch<Goal>(`${environment.apiUrl}/goals/${goal.id}?${queryString}`, goal).pipe(
        map((goal) => createGoal(goal)),
  ...
  async deleteGoal(goal: Goal): Promise<void> {
    await firstValueFrom(this.http.delete(`${environment.apiUrl}/goals/${goal.id}`).pipe(tap(() => this.goalsStore.remove(goal.id))));
  }
}
