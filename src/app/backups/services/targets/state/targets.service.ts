...
export class TargetsService {
  ...
  async getTargets(): Promise<void> {
    await firstValueFrom(
      this.http.get<Target[]>(`${environment.apiUrl}/targets`).pipe(
  ...
  async createNewTarget(target: Target): Promise<Target> {
    return await firstValueFrom(
      this.http.post<Target>(`${environment.apiUrl}/targets`, target).pipe(
  ...
  async updateTarget(target: Target): Promise<Target> {
    return await firstValueFrom(
      this.http.put<Target>(`${environment.apiUrl}/targets/${target.id}`, target).pipe(
        map((t) => createTarget(t)),
