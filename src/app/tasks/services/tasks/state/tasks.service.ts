...
export class TasksService {
  ...
  async getTasks(queryString: string, showLoading = true): Promise<void> {
    ...
    await firstValueFrom(
      this.http.get<Task[]>(`${environment.apiUrl}/tasks?${queryString}`).pipe(
  ...
  async deleteTask(task: Task) {
    await firstValueFrom(this.http.delete(`${environment.apiUrl}/tasks/${task.id}`).pipe(tap(() => this.tasksStore.remove(task.id))));
  }
  ...
  async loadTaskHistoryIfNeeded(task: Task): Promise<void> {
    ...
    await firstValueFrom(
      this.http.get<TaskHistory[]>(`${environment.apiUrl}/tasks/${task.id}/history`).pipe(
