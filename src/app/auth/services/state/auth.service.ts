...
export class AuthService {
  ...
  async login(username: string, password: string) {
    await firstValueFrom(
      this.http
        ...
        .pipe(tap((token) => this.authStorageService.setAuthToken(token.accessToken))),
    );
  }

  async getMe() {
    await firstValueFrom(
      this.http.get<User>(`${environment.apiUrl}/me`).pipe(
  ...
  async updateMe(changes: Partial<User>): Promise<void> {
    const user = this.authQuery.getUser();
    await firstValueFrom(
      this.http.patch<User>(`${environment.apiUrl}/update-me`, { ...user, ...changes }).pipe(
