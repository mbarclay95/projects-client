...
export class FamiliesService {
  ...
  async getFamilies(): Promise<void> {
    await firstValueFrom(
      this.http.get<Family[]>(`${environment.apiUrl}/families`).pipe(
  ...
  async getFamily(familyId: number): Promise<void> {
    await firstValueFrom(
      this.http.get<Family>(`${environment.apiUrl}/families/${familyId}`).pipe(
  ...
  async createNewFamily(family: Family): Promise<void> {
    await firstValueFrom(
      this.http.post<Family>(`${environment.apiUrl}/families`, family).pipe(
        map((family) => createFamily(family)),
  ...
  async updateFamily(familyId: number, family: Partial<Family>): Promise<void> {
    ...
    await firstValueFrom(
      this.http.put<Family>(`${environment.apiUrl}/families/${familyId}`, newFamily).pipe(
        map((family) => createFamily(family)),
